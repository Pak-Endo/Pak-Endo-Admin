...
export class MembersService extends ApiService<user> {
  ...
  deleteUser(memberID: string | null): Observable<ApiResponse<any>> {
    return this.delete(`/user/deleteUser/${memberID}`).pipe(shareReplay(), map((res: ApiResponse<any>) => {
      if(!res.hasErrors()) {
        this.notif.displayNotification('Member successfully removed', 'Remove member', TuiNotification.Success)
        return res.data
      }
      else {
        if (res.errors[0].code && ![401, 403].includes(res.errors[0].code)) {
          return this.notif.displayNotification(res?.errors[0]?.error?.message, 'Remove member', TuiNotification.Error)
        }
      }
    }))
  }
